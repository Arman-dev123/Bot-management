...
export class SidebarComponent implements OnInit {
  readonly navItems: NavItem[] = [
    ...
    { label: 'My Bots', icon: 'smart_toy', route: '/bots' },
    { label: 'Create Bot', icon: 'add_circle_outline', route: '/bots/create' },
    { label: 'Test Panel', icon: 'science', route: '/test-panel', divider: true },
    { label: 'NICE DFO Live Chat', icon: 'support_agent', route: '/dfo-chat' },
    { label: 'Settings', icon: 'settings', route: '/settings', divider: true }
  ];
  ...
  get currentUser() { return this.auth.currentUser(); }
  get userInitial() { return this.currentUser?.name?.charAt(0).toUpperCase() ?? '?'; }
  get dfoChatLoaded() { return this.dfoService.isLoaded(); }

  constructor(
    private auth: AuthService,
    private botService: BotService,
    private dfoService: DfoChatService,
    private router: Router
  ) {}
